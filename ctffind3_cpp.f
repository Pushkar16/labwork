...
C
C**************************************************************************
      SUBROUTINE getparam(AIN,NXYZ,ABOX,JXYZ,IX,DRMS,
     +                    MEAN,RMS,RMSA,NY,CNT,NX)
C**************************************************************************
      IMPLICIT NONE
C
      REAL AIN(*),ABOX(*),DRMS,MEAN,RMS,RMSA(*)
      INTEGER NXYZ(*),JXYZ(*),IX,CNT,NY,NX,I,J,ID,IP
      DO 10 I=1,NY
        IP=(I-1)*JXYZ(2)
        DO 21 J=1,JXYZ(2)
          ID=1+NXYZ(1)*(J-1)
C          print *,'AIN[ID] B4 IS ',AIN(ID)
          CALL IREAD(10,AIN(ID),J+IP)
C          print *,'AIN[ID] AFTER IS ',AIN(ID)
21      CONTINUE
        DO 10 J=1,NX
          IX=(J-1)*JXYZ(1)+1
...
C
C**************************************************************************
      SUBROUTINE calculatepower(AIN,NXYZ,ABOX,JXYZ,IX,DRMS,
     +                    MEAN,RMS,RMSA,NY,CNT,NX,RMSMAX,RMSMIN,
     +                    KXYZ,SCAL,POWER)
C**************************************************************************
      IMPLICIT NONE
C
      REAL AIN(*),ABOX(*),DRMS,MEAN,RMS,RMSA(*)
      REAL RMSMIN,RMSMAX,SCAL,POWER(*)
      INTEGER NXYZ(*),JXYZ(*),KXYZ(*),IX,CNT,NY,NX,I,J,ID,IP,IS,K,L,loop
      INTEGER IERR
      COMPLEX,ALLOCATABLE :: CBOXS(:)
      ALLOCATE (CBOXS(JXYZ(2)),STAT=IERR)
      loop=0

      print *,NY,NX,JXYZ(2),JXYZ(1)/2
      DO 100 I=1,NY
C         PRINT *,'LOOP 1',POWER(1)
         IP=(I-1)*JXYZ(2)
        DO 20 J=1,JXYZ(2)
          ID=1+NXYZ(1)*(J-1)
          CALL IREAD(10,AIN(ID),J+IP)
...
C            PRINT *,'LOOP 2',POWER(1)
C            PRINT *,'LOOP 2 END',POWER(1)
            DO 40 L=1,JXYZ(2)
C              PRINT *,'LOOP 3',POWER(1),loop
              DO 41 K=1,JXYZ(1)/2
                loop=loop+1
                IF (MOD(loop,8192).EQ.1) THEN
C                PRINT *,'LOOP 4',IS,POWER(1),loop
                ENDIF
                ID=(K+JXYZ(1)/2*(L-1))*2
                IS=K+KXYZ(1)*(L-1)
                
                IF (MOD(loop,8192).EQ.1) THEN
C                PRINT *,'just b4 CALC',IS,POWER(1),loop
                ENDIF
                POWER(IS)=POWER(IS)+(ABOX(ID-1)**2+ABOX(ID)**2)*SCAL**2
C                PRINT *,'AFTER',IS,POWER(IS),ID,ABOX(ID),loop
                

41        CONTINUE
              IF (KXYZ(1).GT.JXYZ(1)/2) THEN
C              PRINT *,'IN IF'
              POWER(IS+1)=POWER(IS+1)+ABS(CBOXS(L)*SCAL)**2
              ENDIF
40      CONTINUE
            CNT=CNT+1  
          ENDIF
...
C       PRINT *,'LLOOOOP CONT :::',loop
C       PRINT *,'KXYZ(1) :::',KXYZ(1),'JXYZ(1) ::',JXYZ(1)
C
      RETURN
      END
...
C W[N*(N+3)]
      ...
      F=9.47277762E-43
      print *,'INPUT VALUES'
      PRINT *,'X is',X(1),X(2),X(3)
      PRINT *,'E is',E
      PRINT *,'N is',F
      PRINT *,'ESCALE is',ESCALE
      PRINT *,'IPRINT is',IPRINT
      PRINT *,'ICON is',ICON
      PRINT *,'MAXIT is',MAXIT
      PRINT *,'CS is',CS
      PRINT *,'WL is',WL
      PRINT *,'WGH1 is',WGH1
      PRINT *,'WGH2 is',WGH2
      PRINT *,'THETATR is',THETATR
      PRINT *,'RMIN2 is',RMIN2
      PRINT *,'RMAX2 is',RMAX2
      PRINT *,'NXYZ is',NXYZ
      PRINT *,'HW is',HW
      PRINT *,'DAST is',DAST
      DO 1 I=1,N
...
C      print *,'________________________'
C      print *,'value of NXYZ',NXYZ
...
C      print *,'in 70s'
   ...
   58 K=IDIRN
      DO 9 I=1,N
      X(I)=X(I)+DD*W(K)
C      PRINT *,'VALUE OF DD IS ',DD
C      PRINT *,'VALUE OF W(K) IS',W(K)
      K=K+1
    9 CONTINUE
C      print *,'b4 2nd calcfx',IS
C      print *,'x is',X(1),X(2),X(3)

      CALL CALCFX(N,X,F,AIN,
     +  CS,WL,WGH1,WGH2,THETATR,RMIN2,RMAX2,NXYZ,HW,DAST)
C
C      print *,'after 2nd calcfx',IS
      NFCC=NFCC+1
C      print *,'go to g10',IS
...
C
      ...
      I2=INT(DFMID2/FSTEP)
      ID=(I2-I1+1)*(I2-I1+1)
C      PRINT *,'IN CTF_FIND  ','I1::',I1,'I2::',I2,'ID::',ID
...
C
C      PRINT *,'INPUT IS::',DF1(ID),DF2(ID),ANG(ID)
C      PRINT *,'ID IS ::',ID,' AND SUMS(ID) IS ',SUMS(ID) 
      RETURN
      END      
C**************************************************************************
