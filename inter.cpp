...
int main()
{
	    ...
	    int NXYZ[3], MODE, JXYZ[3], I, J, NX, NY, IX, IS, KXYZ[3];
	    ...
	    float CS, KV, WGH, XMAG, DSTEP, RESMIN, RESMAX, DFMID1, DFMID2;
	    float THETATR, STEPR, RMIN2, RMAX2, HW, RMSMAX, DAST;
	    float RES2, CTF, CTFV, TMP, DFMIN, DFMAX, FSTEP, DRMS1, CMAX;
	    float *AIN, *ABOX, *POWER, *OUT, *BUF1;
	    float *RMSA, *BINS;
	    int boximg_num=1;
	    const float FLT = -0.1;
	    const float PI = 3.1415926535898;
	    //struct s {long double dr,di;}  *CBOXS;
	    char FILEIN[200], FILEOUT[200], TITLE[1600], CFORM, NCPUS[10];
	    char *input;
	    bool EX;
	    //C
	      printf("plc lab\n");
 ...
 printf("Thread rank: %d\n", omp_get_thread_num());
  ...
  std::ifstream myfile ("ctffind3_input.txt");
    ...
    while ( getline (myfile,line) )
    {
	    ...
	    std::string value = line.substr(line.find(delimiter)+1);
		    if(token=="input")
		    {
		    	std::strcpy(FILEIN, value.c_str());
		    	input=&value[0];
		    	std::cout << "input file lenght is"<<strlen(value.c_str())<<"\n";
		    	std::cout << "input file lenght is"<<strlen(input)<<"\n";
		    	std::cout << "input file lenght is"<<strlen(FILEIN)<<"\n";
		    }
		    if(token=="output")
		    {
		    	std::strcpy(FILEOUT, value.c_str());
    ...
    std::cout << "dAst[A] is "<<DAST<< '\n';
 	...
 	std::cout << "size of char array is "<<sizeof(FILEIN)/sizeof(char); 
 	std::string tmp = "OLD";
 	char tab2[1024];
 	...
 	//CBOXS=(float*)calloc(JXYZ[1],sizeof (float));
  ...
  calculatepower_(AIN,NXYZ,ABOX,JXYZ,&IX,&DRMS,&MEAN,&RMS,RMSA,&NY,&CNT,&NX,&RMSMAX,&RMSMIN,KXYZ,&SCAL,POWER);
    /*for(int i=0;i<8000;i++)
      std::cout<<"\n id is "<<i<<" POWER[id] is "<<POWER[i];*/
    
   
   
  
	std::cout<<"\nTotal tiles and number used"<<NX*NY;
	std::cout<<"\nTotal count"<<CNT;
	SCAL=1/(float)CNT;
	std::cout<<"\nTotal SCAL"<<SCAL;
  for(K=0;K<KXYZ[0]*KXYZ[1];K++)
	{
    //std::cout<<"\nold power at "<<K<<" is ::"<<POWER[K]<<"and scal is"<<SCAL;
		POWER[K]=pow(POWER[K]*SCAL,0.5);
    
	}

  std::cout<<"\nRESMIN be4 CALCULATION :::"<<RESMIN;
	STEPR=DSTEP*pow(10,4)/XMAG;
  RESMIN=STEPR/RESMIN;
  RESMAX=STEPR/RESMAX;

    std::cout<<"\nstepr:::"<<RESMIN<<"\t"<<STEPR/50<<"\n";
    if (RESMIN<STEPR/50)
    {
    	RESMIN=STEPR/50;
    ...
    if (RESMIN>RESMAX)
    {
     ...
     CS=CS*pow(10.0,7.0);                     
      ...
      DFMID1=DFMIN;
      DFMID2=DFMAX;
      std::cout<<"\ninput values for search_ctf are::";
      std::cout<<"\n DFMID1::"<<DFMID1<<"\t DFMID2 is "<<DFMID2;
      std::cout<<"\n CS is ::"<<CS<<"\t WL is ::"<<WL;
      std::cout<<"\n WGH1 is ::"<<WGH1<<"\t WGH2 is ::"<<WGH2;
      std::cout<<"\n RESMIN is ::"<<RESMIN<<"\t RESMAX is ::"<<RESMAX;
      std::cout<<"\n THETATR is ::"<<THETATR<<"\t ANGAST is ::"<<ANGAST;
      std::cout<<"\n FSTEP is ::"<<FSTEP<<"\t DAST is ::"<<DAST;
      std::cout<<"\n JXYZ IS ::"<<JXYZ[0]<<JXYZ[1]<<JXYZ[2];
      /*for(int i=0;i<8000;i++)
        std::cout<<"\n id is "<<i<<" POWER[id] is "<<POWER[i];*/
      	    ...
      	    if (OUT[IS]<-1.0) 
      	   	{
      	   	 ...
      	   	 RES2=pow((float)LL/JXYZ[1],2)+pow((float)MM/JXYZ[1],2);
    ...
    // +           JXYZ(3),'NEW',STEPR,TITLE)
              tmp="NEW";
              strcpy(tab2, tmp.c_str());
              iopen_(FILEOUT,&IFILE,&CFORM,&MODE,&NXYZ[0],&NXYZ[1],&NXYZ[2],tab2,&STEPR,TITLE);
