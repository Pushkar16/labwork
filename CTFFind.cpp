...
CTFFind::CTFFind(char FILEIN[],char FILEOUT[],float CS,float KV,float WGH,float XMAG,float DSTEP,float RESMIN,float RESMAX,float DFMIN,float DFMAX,float FSTEP,float DAST,int Box)
{
    ...
    *LL=L-1;
    for (M = 1; M <= JXYZ[1]; M++)
    {
      *MM=M-1;
      ...
      int RES2=pow((float)*LL/JXYZ[1],2)+pow((float)*MM/JXYZ[1],2);
